...
pdfQueue.process(async (job) => {
  ...
  console.log(`[PDF Worker] Generating PDF for order ${order._id}`);
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      writeStream.on('finish', () => {
        console.log(`[PDF Worker] Successfully generated PDF at ${filePath}`);
        resolve({ filePath });
      });

      writeStream.on('error', (err) => {
        console.error(`[PDF Worker] Stream error:`, err);
        reject(err);
      });
