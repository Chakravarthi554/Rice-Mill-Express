...
export default function RegisterScreen({ navigation }) {
    ...
    const handleRegister = async () => {
        const { name, email, password, confirmPassword, phone, role, sellerType, referralCode } = formData;
        if (!/^\d{10}$/.test(phone)) {
        ...
        try {
            ...
            await dispatch(register({ name, email, password, phone, role, sellerType, referralCode, deviceId })).unwrap();
                ...
                <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                    ...
                    <View style={styles.formCard}>
                        ...
                        {formData.role === 'seller' && (
                            <View style={[styles.inputContainer, { paddingHorizontal: 0, paddingVertical: Platform.OS === 'ios' ? 4 : 0 }]}>
                                <Feather name="briefcase" size={18} color={COLORS.textMuted} style={{ marginLeft: 16 }} />
                                <Picker
                                ...
                                >
                                    ...
                                    <Picker.Item label="Rice Mill" value="Rice Mill" />
                                    <Picker.Item label="Farmer Producer Organization (FPO)" value="Farmer Producer Organization (FPO)" />
                                    <Picker.Item label="Wholesaler" value="Wholesaler" />
                                    <Picker.Item label="Distributor" value="Distributor" />
                                    <Picker.Item label="Farmer Cooperative" value="Farmer Cooperative" />
