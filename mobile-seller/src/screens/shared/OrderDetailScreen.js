...
import {
    ...
    Alert, TouchableOpacity, Linking, Image, SafeAreaView, StatusBar, Share, TextInput, Modal
} from 'react-native';
...
const OrderDetailScreen = ({ route, navigation }) => {
    ...
    const handleRefundRequest = () => {
        Alert.alert("Return / Refund", "Please contact support to initiate a return or refund for this order.", [{ text: "OK" }]);
    };
    ...
    const handleCancelOrder = async () => {
        ...
        try {
            ...
            await apiService.updateOrderStatus(order._id, 'cancelled', cancelReason);
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
                ...
                {/* ── SELLER ACTIONS ── */}
                {user?.role === 'seller' && (order.orderStatus === 'pending' || order.orderStatus === 'placed') && (
                    <SectionCard>
                        <SectionHeader icon="settings" title="Seller Actions" color="#EF4444" />
                        <TouchableOpacity 
                            style={[styles.refundBtn, { backgroundColor: '#FEE2E2', borderColor: '#F87171' }]} 
                            onPress={() => setCancelModalVisible(true)}
                        >
                            <Feather name="x-circle" size={16} color="#EF4444" />
                            <Text style={[styles.refundBtnText, { color: '#EF4444' }]}>Cancel Order</Text>
                        </TouchableOpacity>
                    </SectionCard>
                )}
                ...
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <View style={styles.modalActions}>
                            ...
                            <TouchableOpacity style={styles.modalBtnSubmit} onPress={handleCancelOrder} disabled={actionLoading}>
                                {actionLoading ? <ActivityIndicator color="#fff" /> : <Text style={styles.modalBtnSubmitText}>Confirm Cancel</Text>}
                            </TouchableOpacity>
                        </View>
                    </View>
                </View>
...
const styles = StyleSheet.create({
    ...
    partnerPhoneLabel: { fontSize: 12, color: '#9CA3AF', marginBottom: 2 },
    partnerPhoneValue: { fontSize: 14, color: '#111827', fontWeight: '500' },
    ...
    reasonInput: { borderWidth: 1, borderColor: '#D1D5DB', borderRadius: 8, padding: 12, fontSize: 16, minHeight: 100, textAlignVertical: 'top', marginBottom: 20 },
    modalActions: { flexDirection: 'row', justifyContent: 'flex-end', gap: 12 },
    modalBtnCancel: { paddingVertical: 10, paddingHorizontal: 16, borderRadius: 8 },
    modalBtnCancelText: { color: '#6B7280', fontWeight: '600', fontSize: 16 },
    modalBtnSubmit: { backgroundColor: '#EF4444', paddingVertical: 10, paddingHorizontal: 16, borderRadius: 8, minWidth: 100, alignItems: 'center' },
    modalBtnSubmitText: { color: '#fff', fontWeight: '600', fontSize: 16 },
