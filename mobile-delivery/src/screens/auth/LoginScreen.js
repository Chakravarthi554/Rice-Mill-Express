...
export default function LoginScreen({ navigation }) {
    ...
    useEffect(() => {
        GoogleSignin.configure({
            webClientId: process.env.EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID || '381785078247-kqln1mulv78qq1te9ipvtgcr7rkd5sd0.apps.googleusercontent.com',
            offlineAccess: false,
        });
    }, []);
    ...
    const handleGoogleLogin = async () => {
        ...
        try {
            ...
            if (result && result.requires2FA) { navigation.navigate('TwoFactorVerify', { userId: result.userId }); }
        } catch (error) {
            if (error !== 'Google sign in cancelled') {
                Alert.alert('Google Sign-In Failed', error || 'An error occurred during Google login');
            }
        }
    };
