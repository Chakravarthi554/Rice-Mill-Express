...
export const apiService = {
    ...
    // ============ Delivery Partner ============
    toggleDutyStatus: (isOnline) => api.post('/api/v1/dp/update-activity', { isOnline, action: isOnline ? 'online' : 'offline' }),
    ...
    requestReplacement: (orderId, formData) =>
        api.post(`/api/v1/dp/request-replacement/${orderId}`, formData, {
            headers: { 'Content-Type': 'multipart/form-data' },
            timeout: 30000,
        }),
    ...
    getDeliveryHistory: () => api.get('/api/v1/dp/history'),
